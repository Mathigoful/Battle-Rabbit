...
public class Jumpers : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.transform.tag == "Player")
        {
            ...
            FindObjectOfType<AudioM>().Play("Bumper");
            _pRigidBody.AddForce(new Vector3(_pRigidBody.velocity.x, _jumpersForce));
        ...
        if (collision.transform.tag == "Player1")
        {
            ...
            _pRigidBody2.AddForce(new Vector3(_pRigidBody2.velocity.y, _jumpersForce));
            Debug.Log("Jumper Activated !");
